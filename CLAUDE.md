...
- **The ESP is listmonk, not Brevo.** `apps/web/src/lib/brevo.ts` and its callers (the signup route, the reconciler, `route.test.ts`) target the Brevo API, but **Brevo was never provisioned and is not wanted**. `packages/infra/alchemy.run.ts` binds the four `BREVO_*` values as **empty strings on purpose**, which makes `readBrevoConfig` return `null` — the signal the route already fails closed on (503) and the reconciler no-ops on. Treat that code as a placeholder awaiting replacement, not as a working integration. The real ESP is **self-hosted listmonk v5** at `lists.mrdemonwolf.com` (Dokploy on ash-01, sending via Amazon SES us-east-1); nothing in this repo talks to it yet. The swap is `POST /api/subscribers` then `POST /api/subscribers/{id}/optin` with a listmonk API token — `/api` sits outside Cloudflare Access deliberately, so no Access service token is needed. Replace the `BREVO_*` secrets in `alchemy.run.ts` and restore them to the deploy workflow's required-configuration check at the same time. Note listmonk v5 supports only hCaptcha and Altcha, **not Turnstile** — Turnstile stays where it is, verified by our own route, and does not move to listmonk.
...
Of those, only `TURNSTILE_SECRET_KEY` and `ALCHEMY_PASSWORD` carry real values. The four `BREVO_*` entries are bound as **empty strings on purpose** — there is no Brevo account and there never was. Do not "fix" them by inventing values or by wiring a Brevo signup; they get deleted and replaced by listmonk credentials when that swap lands.
