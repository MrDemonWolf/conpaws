...
const SMALL_ICS_PATH = path.resolve(__dirname, '../../../../../test-data/small-test.ics');
...
describe('parseIcs', () => {
  ...
  it('parses basic event fields (title, uid)', () => {
    const result = parseIcs(loadSmallIcs());
    const opening = result.events.find((e) => e.sourceUid === 'test-opening-001');
  ...
  it('parses event source URL', () => {
    const result = parseIcs(loadSmallIcs());
    const opening = result.events.find((e) => e.sourceUid === 'test-opening-001');
    expect(opening!.sourceUrl).toBe('https://testcon2026.sched.com/event/test-opening-001');
  });
...
END:VCALENDAR`;
    ...
    expect(result.events[0].description).toContain('folded onto multiple lines');
  });
  ...
  it('deduplicates categories and assigns colors', () => {
    ...
    const convServices = result.categories.find((c) => c.name === 'CONVENTION SERVICES');
