...
export default ({ config }: ConfigContext): ExpoConfig => ({
  ...
  plugins: [
    ...
    // Turns on R8 for release builds. The template's build.gradle reads
    // `android.enableMinifyInReleaseBuilds` and defaults it to false, so every
    // build so far has shipped unminified. Set from a plugin because android/ is
    // regenerated by every prebuild. Play picks the mapping out of the bundle
    // automatically, so this also clears the console's deobfuscation warning.
    "./plugins/withReleaseMinify.js",
